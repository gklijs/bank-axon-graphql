...
class BankTransferManagementSaga {
    ...
    protected fun on(event: TransferStartedEvent) {
        ...
        val command: Any = if (this.to.equals(this.from)) {
            MarkTransferFailedCommand(this.transferId, BankExceptionStatusCode.FROM_AND_TO_SAME.description)
            ...
            DebitMoneyCommand(this.from, this.token, this.amount, this.username, this.transferId)
        }
        commandBus!!.dispatch(
    ...
    fun on(event: MoneyDebitedEvent) {
        this.debited = true
        val command: Any = if (IbanUtil.isValidOpenIban(this.to)) {
            CreditMoneyCommand(this.to, this.amount, this.transferId)
        } else {
    ...
    fun on(event: MoneyReturnedEvent) {
        val command = MarkTransferFailedCommand(this.transferId, event.reason)
        commandBus!!.dispatch(
            GenericCommandMessage.asCommandMessage(command),
            ErrorHandlingCallback { r -> this.handleError(r) })
    }
    ...
    class ErrorHandlingCallback(
        ...
        override fun onResult(message: CommandMessage<*>, commandResultMessage: CommandResultMessage<*>) {
            if (commandResultMessage.isExceptional) {
                ...
                var reason = commandResultMessage.exceptionResult().message
                    ?: BankExceptionStatusCode.UNKNOWN_EXCEPTION.description
                if (reason == "The aggregate was not found in the event store") {
                    reason = BankExceptionStatusCode.OPEN_BANK_ACCOUNT_NOT_FOUND.description
                } else {
                    reason = BankExceptionStatusCode.validDescription(reason);
                }
