    ...
    (let [new-sub-id (inc (:subscription-id db))]
      (if-let [ats (:active-t-subscription db)]
        [[::re-graph/unsubscribe (keyword (str "transactions-" ats))]
         [::re-graph/query query-query arguments-map [:tech.gklijs.bank.events/reset-transactions]]
         [::re-graph/subscribe (keyword (str "transactions-" new-sub-id)) subscription-query arguments-map [:tech.gklijs.bank.events/on-transaction]]
